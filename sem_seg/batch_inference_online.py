...
"python batch_inference.py --path_data a/b/c --path_cls meta/class_or.txt --model_path RUNS/test_indoor --dump_dir RUNS/test_indoor --test_name aaaaa/bbb  --out"
        ...
        for root, dirs, files in os.walk(path_test):  # for each folder
            ...
            for file in enumerate(files):  # for each file in the folder
                ...
                if re.search("\.(npy)$", file[1]):  # if the file is a npy
                    ...
                    eval_one_epoch(sess, ops, filepath, out_data_label_filename, out_gt_label_filename)
